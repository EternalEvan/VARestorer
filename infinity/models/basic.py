...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, attn_bias_or_two_vector: Union[torch.Tensor, Tuple[torch.IntTensor, torch.IntTensor]], attn_fn=None, scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0):
        ...
        # Use the module call rather than accessing .weight/.bias directly so that
        # LoRA-wrapped nn.Linear (LoraInjectedLinear) works correctly.
        # Produce the same concatenated bias as original implementation.
        bias_qkv = torch.cat((self.q_bias, self.zero_k_bias, self.v_bias))
        # If mat_qkv is a plain nn.Linear, calling it is equivalent to F.linear with its weight and bias.
        qkv = self.mat_qkv(x)
        # If the linear had no bias (but original code expects the concatenated bias), add it.
        if (not hasattr(self.mat_qkv, 'bias')) or (self.mat_qkv.bias is None):
            qkv = qkv + bias_qkv.unsqueeze(0).unsqueeze(0)
        qkv = qkv.view(B, L, 3, self.num_heads, self.head_dim)  # BL3Hc

        # #my code
        # #important
        # from lora_diffusion import LoraInjectedLinear
        # if isinstance(self.mat_kv, LoraInjectedLinear):
        #     kv_compact = self.mat_kv(kv_compact).view(N, 2, self.num_heads, self.head_dim)
        # else:     
