...
class Infinity(nn.Module):
    ...
    def forward(self, 
        ...
        x_BLC_wo_prefix: torch.Tensor, scale_schedule: List[Tuple[int]],
        cfg_infer=False,
        **kwargs,
...
class CInfinity(Infinity):
    ...
    def forward(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC_wo_prefix: torch.Tensor, scale_schedule: List[Tuple[int]],
        cfg_infer=False,control_tensors=None,
        **kwargs,
    ) -> Union[torch.Tensor, List[torch.Tensor]]:  # returns logits_BLV
        """
        label_B_or_BLT: label_B or (kv_compact, cu_seqlens_k, max_seqlen_k)
        ...
        if self.num_block_chunks == 1:
            for i, b in enumerate(self.car_blocks):
                ...
                if not self.add_lvl_embeding_only_first_block:
                    car_input = self.add_lvl_embeding_for_x_BLC(car_input, scale_schedule, need_to_pad)
                if checkpointing_full_block:
                    car_input = torch.utils.checkpoint.checkpoint(b, car_input, cond_BD_or_gss, ca_kv, attn_bias_or_two_vector, attn_fn, scale_schedule, self.rope2d_freqs_grid, use_reentrant=False)
                else:
                    car_input = b(x=car_input, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=attn_bias_or_two_vector, attn_fn=attn_fn, scale_schedule=scale_schedule, rope2d_freqs_grid=self.rope2d_freqs_grid)
            ...
            for i, chunk in enumerate(self.car_block_chunks): # this path
                ...
                if not self.add_lvl_embeding_only_first_block:
                    car_input = self.add_lvl_embeding_for_x_BLC(car_input, scale_schedule, need_to_pad)
                car_input = chunk(x=car_input, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=attn_bias_or_two_vector, attn_fn=attn_fn, scale_schedule=scale_schedule, checkpointing_full_block=checkpointing_full_block, rope2d_freqs_grid=self.rope2d_freqs_grid)
                control_residual_f.append(car_input)
        ...
        if self.num_block_chunks == 1:
            ...
            for i, chunk in enumerate(self.block_chunks): # this path
                if self.add_lvl_embeding_only_first_block and i == 0:
                    x_BLC = self.add_lvl_embeding_for_x_BLC(x_BLC, scale_schedule, need_to_pad)
                if not self.add_lvl_ekbeding_only_first_block:
                    x_BLC = self.add_lvl_embeding_for_x_BLC(x_BLC, scale_schedule, need_to_pad)
