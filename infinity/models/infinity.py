...
class STGumbelArgmax(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        # 反向传播：用 Gumbel-Softmax 的软概率梯度替代硬 one-hot 的梯度
...
class Infinity(nn.Module):
    ...
    def autoregressive_infer_cfg_w_lq_token(
        ...
        patch_nums_per_level = [pn[0]*pn[1]*pn[2] for pn in scale_schedule]  # note important pn[0]==1?
    ...
    def logits_to_img(self,logits_BlV_all,vae,scale_schedule,top_k=900,top_p=0.97,g_seed=1):
        ...
        for si,logits_BlV in enumerate(logits_BlV_list): 
            ...
            if self.use_bit_label:
                ...
                # #####
                # idx_Bld = STGumbelArgmax.apply(logits_BlV, 0.5)
                # tmp_tensor = torch.zeros_like(idx_Bld).to(idx_Bld.device)
                # tmp_tensor[:,:,1:]=1
                # idx_Bld = idx_Bld * tmp_tensor
                # idx_Bld = idx_Bld.sum(dim=-1)
...
class BInfinity(nn.Module):   ###backbone
    def __init__(
        self, vae_local,
        text_channels=0, text_maxlen=0,     # text-cond generation
        selecting_idx=None,                 # class-cond generation
        ...
        **kwargs,
        ...
        if self.num_block_chunks == 1:
            ...
            for i, chunk in enumerate(self.block_chunks): # this path
                ...
                if not self.add_lvl_embeding_only_first_block:
                    x_BLC = self.add_lvl_embeding_for_x_BLC(x_BLC, scale_schedule_new, need_to_pad)
                #chunk scale_schedule is for selecting rotatory embedding
        ...
        #return self.get_logits(x_BLC[:, :l_end], cond_BD)    # return logits BLV, V is vocab_size
        return self.get_logits(x_BLC[:, :all_scale_length], cond_BD)    # return logits BLV, V is vocab_size
...
class AInfinity(nn.Module):  # x_BLC add x_BLC_lq
    def __init__(
        self, vae_local,
        text_channels=0, text_maxlen=0,     # text-cond generation
        selecting_idx=None,                 # class-cond generation
    ...
    def autoregressive_infer_cfg(
        ...
        # x_BLC = torch.cat((last_stage, self.word_embed(self.norm0_ve(x_BLC_wo_prefix.float()))), dim=1)
...
class FAInfinity(nn.Module):  # x_BLC add x_BLC_lq
    def __init__(
        self, vae_local,
        text_channels=0, text_maxlen=0,     # text-cond generation
        selecting_idx=None,                 # class-cond generation
    ...
    def autoregressive_infer_cfg(
        ...
        index = torch.zeros((bs,)).to(x_BLC.device)  ###change from torch.ones to torch.zeros
        t_emb = dist.timestep_embedding(index, self.C//4, repeat_only=False)
        t_emb = self.time_embed(t_emb)
...
class FInfinity(nn.Module):
    def __init__(
        ...
        # self.car_control_convs = ControlConditionEmbedding_patch_size_32(conditioning_embedding_channels=self.C)
    ...
    # def forward(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC_wo_prefix: torch.Tensor, scale_schedule: List[Tuple[int]],
    #     cfg_infer=False,x_BLC_w_prefix_lq=None,index=None,
    #     **kwargs,
    # ) -> Union[torch.Tensor, List[torch.Tensor]]:  # returns logits_BLV
    #     """
    #     label_B_or_BLT: label_B or (kv_compact, cu_seqlens_k, max_seqlen_k)
    ...
    #                 x_BLC = torch.utils.checkpoint.checkpoint(b, x_BLC, cond_BD_or_gss, ca_kv, None, None, scale_schedule, self.rope2d_freqs_grid, use_reentrant=False)
    #             else:
    #                 x_BLC = b(x=x_BLC, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=None, attn_fn=attn_fn, scale_schedule=scale_schedule, rope2d_freqs_grid=self.rope2d_freqs_grid)
    #     else:
    #         for i, chunk in enumerate(self.block_chunks): # this path
    #             if self.add_lvl_embeding_only_first_block and i == 0:
    #                 x_BLC = self.add_lvl_embeding_for_x_BLC(x_BLC, scale_schedule, need_to_pad)
    ...
    #             x_BLC = torch.cat([x_BLC,x_BLC_lq],dim = 1)
    #             x_BLC = x_BLC + t_emb.unsqueeze(1)          
    #             x_BLC = chunk(x=x_BLC, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=None, attn_fn=attn_fn, scale_schedule=scale_schedule, checkpointing_full_block=checkpointing_full_block, rope2d_freqs_grid=self.rope2d_freqs_grid)
    #     # [3. unpad the seqlen dim, and then get logits]
    #     return self.get_logits(x_BLC[:, :l_end], cond_BD)    # return logits BLV, V is vocab_size

    # def forward(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC_wo_prefix: torch.Tensor, scale_schedule: List[Tuple[int]],
    #     cfg_infer=False,raw_features_seq=None,index=None,  #here raw_features_seq is quantized_raw_features_seq
    #     **kwargs,
    # ) -> Union[torch.Tensor, List[torch.Tensor]]:  # returns logits_BLV
    #     """
    #     label_B_or_BLT: label_B or (kv_compact, cu_seqlens_k, max_seqlen_k)
    ...
    #     x_BLC_wo_prefix = x_BLC_wo_prefix.float()       # input should be float32
    #     raw_features_seq = raw_features_seq.float()
    #     B = x_BLC_wo_prefix.shape[0]
    ...
    #         sos = sos.unsqueeze(1).expand(B, 1, -1) + self.pos_start.expand(B, 1, -1)
    #         x_BLC = torch.cat((sos, self.word_embed(self.norm0_ve(x_BLC_wo_prefix))), dim=1)
    #         x_BLC_lq = self.word_embed_lq(self.norm0_ve_lq(raw_features_seq))
    ...
    def forward(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC_wo_prefix: torch.Tensor, scale_schedule: List[Tuple[int]],
        cfg_infer=False,lq_images=None,index=None,  #here raw_features_seq is quantized_raw_features_seq
        **kwargs,
    ) -> Union[torch.Tensor, List[torch.Tensor]]:  # returns logits_BLV
        """
        label_B_or_BLT: label_B or (kv_compact, cu_seqlens_k, max_seqlen_k)
...
class CInfinity(Infinity):
    def __init__(
        ...
        super(CInfinity,self).__init__(vae_local,
            text_channels, text_maxlen,     # text-cond generation
            selecting_idx,                 # class-cond generation
            embed_dim, depth, num_heads, mlp_ratio,   # model's architecture
            drop_rate, drop_path_rate,    # drop out and drop path
            norm_eps, rms_norm,      # norm layer
            shared_aln, head_aln,    # adaptive norm
            cond_drop_rate,                 # for classifier-free guidance
            rand_uncond,
        ...
        # for _ in range(depth // 2):
        #     car_skip_norm.append(car_norm_layer(self.C, elementwise_affine=True, eps=1e-6))
        #     car_skip_linear.append(nn.Linear(self.C, self.C))
    ...
    def forward(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC_wo_prefix: torch.Tensor, scale_schedule: List[Tuple[int]],
        ...
        **kwargs,
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            # car_input = []
            # var_x = sos.transpose(1, 2).contiguous().reshape(B, self.C, self.raw_scale_schedule[0], self.raw_scale_schedule[0])
            ...
            # for si, (pn, var_input) in enumerate(zip(self.raw_scale_schedule[1:], x_BLC_wo_prefix)):
            ...
            # x_BLC_lq = x_BLC_lq + x_BLC
            # note important only for 512*512
            patch_nums_per_level = [pn[0]*pn[1]*pn[2] for pn in scale_schedule]  # note important pn[0]==1?
            x_BLC_lq_list = list(torch.split(x_BLC_lq,patch_nums_per_level,dim=1))
            x_BLC_list = list(torch.split(x_BLC,patch_nums_per_level,dim=1))
            x_BLC_lq_list_new = []
        ...
        if self.num_block_chunks == 1:
            ...
            for i, chunk in enumerate(self.car_block_chunks): # this path
                if self.add_lvl_embeding_only_first_block and i == 0:
                    x_BLC_lq = self.add_lvl_embeding_for_x_BLC(x_BLC_lq, scale_schedule, need_to_pad)
                if not self.add_lvl_embeding_only_first_block:
                    x_BLC_lq = self.add_lvl_embeding_for_x_BLC(x_BLC_lq, scale_schedule, need_to_pad)
        ...
        if self.num_block_chunks == 1:
            ...
            for i, chunk in enumerate(self.block_chunks): # this path
                ...
                #     con_f = control_residual_f.pop()
                #     # cat = torch.cat([x_BLC, con_f], dim=-1)
                #     con_f = self.car_skip_norm[i - len(self.block_chunks) // 2](con_f)
                #     con_f = self.car_skip_linear[i - len(self.block_chunks) // 2](con_f)
                #     x_BLC = x_BLC + con_f       
    ...
    def car_inference(
        ...
        if any(np.array(cfg_list) != 1):
            ...
            if not negative_label_B_or_BLT:
                ...
                #my code
                #FSDP flattening
                # if self.cfg_uncond.ndim ==1:
                #     last_dim =  kv_compact_un.shape[-1]
                #     cfg_uncond = self.cfg_uncond.reshape(-1,last_dim)
                for le in lens:
                    kv_compact_un[total:total+le] = (self.cfg_uncond)[:le]
                    total += le
                kv_compact = torch.cat((kv_compact, kv_compact_un), dim=0)
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            ...
            if x_BLC_lq_list is not None:
                ...
                for i, chunk in enumerate(self.car_block_chunks): # this path
                    ...
                    if not self.add_lvl_embeding_only_first_block:
                        control_x = self.add_lvl_embeding(control_x, si, scale_schedule, need_to_pad=need_to_pad)
                    # for m in chunk.module:  #used for FSDP
                    ...
                    #chunk(x=x_BLC_lq, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=attn_bias_or_two_vector, attn_fn=attn_fn, scale_schedule=scale_schedule, checkpointing_full_block=checkpointing_full_block, rope2d_freqs_grid=self.rope2d_freqs_grid)
                    control_residual_f.append(control_x)
            ...
            for block_idx, b in enumerate(self.block_chunks):
                ...
                #####
                #my code
                #for m in b.module: need to change
                for m in b.module:  #used for FSDP
                    last_stage = m(x=last_stage, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=None, attn_fn=attn_fn, scale_schedule=scale_schedule, rope2d_freqs_grid=self.rope2d_freqs_grid, scale_ind=si)
                    if (cfg != 1) and (layer_idx in abs_cfg_insertion_layers):
                        # print(f'add cfg={cfg} on {layer_idx}-th layer output')
                        last_stage = cfg * last_stage[:B] + (1-cfg) * last_stage[B:]
...
class CInfinity2(Infinity):
    ...
    def forward_img_infinity(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC_wo_prefix: torch.Tensor, scale_schedule: List[Tuple[int]],
        ...
        **kwargs,
        ...
        # [3. unpad the seqlen dim, and then get logits]
        logit_BlV = self.get_logits(x_BLC[:, :l_end], cond_BD)  #note tau_list
        if self.use_bit_label:
            tmp_bs, tmp_seq_len = logits_BlV.shape[:2]
            logits_BlV = logits_BlV.reshape(tmp_bs, -1, 2)
            idx_Bld = sample_with_top_k_top_p_also_inplace_modifying_logits_(logits_BlV, rng=rng, top_k=top_k or self.top_k, top_p=top_p or self.top_p, num_samples=1)[:, :, 0]
        ...
        # img = (img + 1) / 2
        # img = img.permute(0, 2, 3, 1).mul_(255).to(torch.uint8).flip(dims=(3,))
        return    # return logits BLV, V is vocab_size
    ...
    def car_inference(
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            ...
            for block_idx, b in enumerate(self.block_chunks):
                ...
                #my code
                for m in b.module:
                # for m in b.module.module:  #used for FSDP
                    last_stage = m(x=last_stage, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=None, attn_fn=attn_fn, scale_schedule=scale_schedule, rope2d_freqs_grid=self.rope2d_freqs_grid, scale_ind=si)
                    if (cfg != 1) and (layer_idx in abs_cfg_insertion_layers):
                        # print(f'add cfg={cfg} on {layer_idx}-th layer output')
                        last_stage = cfg * last_stage[:B] + (1-cfg) * last_stage[B:]
...
# ) -> torch.Tensor:
#     B, l, V = logits_BlV.shape
#     probs = logits_BlV.softmax(dim=-1)  # 可导的概率分布

#     # 前向传播取离散索引
#     _, indices = probs.max(dim=-1)      # (B, l)

#     # STE技巧：前向传播用离散索引，反向传播用概率梯度
#     one_hot = torch.zeros_like(probs).scatter_(-1, indices.unsqueeze(-1), 1.0)  # (B, l, V)
#     ste_vals = probs + (one_hot - probs).detach()  # STE桥接

#     # 返回索引（实际使用时可只返回indices，ste_vals仅用于梯度）
#     return ste_vals.argmax(dim=-1)  # (B, l)
