...
from infinity.dataset.build import build_t2i_dataset,build_sr_dataset
...
from tools.run_varestorer import encode_prompt
...
def gen_one_img(
    ...
    tau_list=[],
    negative_prompt='',
    scale_schedule=[(1, 1, 1), (1, 2, 2), (1, 4, 4), (1, 6, 6), (1, 8, 8), (1, 12, 12), (1, 16, 16), (1, 20, 20), (1, 24, 24), (1, 32, 32)],
    top_k=900,
    top_p=0.97,
    cfg_sc=3,
    cfg_exp_k=0.0,
    ...
    #note important with self.gpt_opt.amp_ctx
    ...
    with torch.amp.autocast('cuda', enabled=False):
        with torch.no_grad():
            ...
            raw_features_lq, _, _ = vae.encode_for_raw_features(lq_images, scale_schedule=vae_scale_schedule)
            
    x_BLC_wo_prefix_lq,_ = bitwise_self_correction.flip_requant(vae_scale_schedule, lq_images, raw_features_lq, args.device)
...
def build_everything_from_args(args: arg_util.Args, saver):
    ...
    # IMPORTANT: import heavy package `BInfinityTrainer` after the Dataloader object creation/iteration to avoid OOM
    from trainer import BInfinityTrainer
    # build trainer
    trainer = BInfinityTrainer(
...
def build_model_optimizer(args, vae_ckpt):
    ...
    from infinity.utils.load import build_vae_bgpt
    
    # disable builtin initialization for speed
    setattr(torch.nn.Linear, 'reset_parameters', lambda self: None)
    setattr(torch.nn.LayerNorm, 'reset_parameters', lambda self: None)
    vae_local, gpt_wo_ddp, gpt_wo_ddp_ema = build_vae_bgpt(args, vae_ckpt, skip_gpt=False, device=args.model_init_device)
    del vae_ckpt
    if args.tini < 0:
        args.tini = math.sqrt(1 / gpt_wo_ddp.C / 3)
    init_weights(gpt_wo_ddp, other_std=args.tini)
    
    # print(gpt_wo_ddp.train_names)
    # print(list(name for name,_ in gpt_wo_ddp.named_parameters()))

    gpt_wo_ddp.special_init(aln_init=args.aln, aln_gamma_init=args.alng, scale_head=args.hd0, scale_proj=args.diva)

    # ### resume training from model with lora injection
    # #####
    # for p in gpt_wo_ddp.parameters():
    #     p.requires_grad_(False)    
    # unet_lora_params, train_names = inject_trainable_lora(gpt_wo_ddp.block_chunks, r=32)
    # #####
    ...
    if args.rush_resume:
        ...
        if 'trainer' in cpu_d:
            ...
            print(gpt_wo_ddp.load_state_dict(state_dict, strict=False))
            if args.use_fsdp_model_ema:
                gpt_wo_ddp_ema.load_state_dict(ema_state_dict, strict=False)
                
        elif 'infinity' in cpu_d:
            state_dict = cpu_d['infinity']
            ema_state_dict = cpu_d['infinity']
            gpt_wo_ddp.load_state_dict(state_dict, strict=True)
            if args.use_fsdp_model_ema:
                gpt_wo_ddp_ema.load_state_dict(ema_state_dict, strict=True)
                
        else:
            state_dict = cpu_d
            ema_state_dict = state_dict
            print(gpt_wo_ddp.load_state_dict(state_dict, strict=True))
            if args.use_fsdp_model_ema:
                gpt_wo_ddp_ema.load_state_dict(ema_state_dict, strict=True)
                
    ## resume training from model without lora injection
    ####
    for p in gpt_wo_ddp.parameters():
        p.requires_grad_(False)    
    ### my code
    ...
    for p in vae_local.parameters():
        p.requires_grad_(False)
    # unet_lora_params_vae, train_names_vae = inject_trainable_lora_extended(vae_local.encoder)
    ndim_dict_vae = {name: para.ndim for name, para in vae_local.named_parameters() if para.requires_grad}
    ...
    # my code 
    trainable_gpt_params = sum(p.numel() for p in gpt_wo_ddp.parameters() if p.requires_grad)
    print(f'[PT][#trainable_para] GPT={trainable_gpt_params / 1e6:.2f}M')

    # my code 
    frozen_gpt_params = sum(p.numel() for p in gpt_wo_ddp.parameters() if not p.requires_grad)
    print(f'[PT][#frozen_para] GPT={frozen_gpt_params / 1e6:.2f}M')
    ...
    # names_vae, paras_vae, para_groups_vae = filter_params(vae_local, ndim_dict_vae, nowd_keys)
    # para_groups = para_groups + para_groups_vae
    del ndim_dict
    if '_' in args.ada:
    ...
    #gpt_optim = AmpOptimizer('gpt', args.fp16, opt_clz(params=para_groups, **opt_kw), gpt_ddp if args.zero else gpt_wo_ddp, args.r_accu, args.tclip, args.zero, vae_local)
...
def build_dataloaders(args):
    if args.task_type == 'sr':
        #my code
        dataset_train = build_sr_dataset(
            args, 
            args.data_path, 
            args.data_load_reso, 
...
def main_train(args: arg_util.Args):
    ...
    sd = torch.load(args.swinir_path, map_location="cpu")
    ...
    from trainer import BInfinityTrainer
    ret: Tuple[
        misc.TensorboardLogger, T5TokenizerFast, T5EncoderModel, BInfinityTrainer,
    ...
    #embedding parameters, alignment parameters, other parameters 
    ...
    # build wandb logger
    if dist.is_master():
        os.makedirs('visualize_train', exist_ok=True)
    ...
    with misc.Low_GPU_usage(files=[args.log_txt_path], sleep_secs=20, verbose=True) as telling_dont_kill:
        ...
        for it, data in me.log_every(start_it, iters_train, ld_or_itrt, args.log_freq, args.log_every_iter, header):
            ...
            if (g_it+1) % args.save_model_iters_freq == 0:
                ...
                # with misc.Low_GPU_usage(files=[args.log_txt_path], sleep_secs=3, verbose=True):
                #     saver.sav_w_vae(args=args, g_it=(g_it+1), next_ep=ep, next_it=it+1, trainer=trainer, acc_str=f'[todo]', eval_milestone=None, also_save_to=None, best_save_to=None)

            with maybe_record_function('before_train'):
                # [get data]
                inp, captions, lq_images = data
                #inp:[4,3,256,256] [-1,1]
                
                #my code
                lq_images = lq_images.to(args.device, non_blocking=True)
                lq_images = (lq_images+1)/2
                
                with torch.no_grad():
                    lq_images = lq_images.clamp(0.0, 1.0)
                    lq_images = swinir(lq_images)
                # print(f"lq_images min max {lq_images.min().item()} {lq_images.max().item()}")
                lq_images = lq_images.clamp(0.0, 1.0)
                lq_images = lq_images * 2 - 1   # [-1,1]
